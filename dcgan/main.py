...
flags.DEFINE_string("dataset", 'new', "The name of dataset (used to name folders etc)")
...
flags.DEFINE_string("sample_dir", 'samples/new', "Directory name to save the image samples [samples]")
flags.DEFINE_string("data_dir", "./data/LLD-icon.hdf5", "Directory path with favicon data")
...
flags.DEFINE_integer("z_dim", 512, "Dimension of Z-vector [100]")
...
flags.DEFINE_string("labels", 'labels/resnet/rc_128', "Specifies the HDF5 path for the labels, if any")
flags.DEFINE_string("sampling", 'uniform', "Sampling method (uniform/normal/t-normal/sphere) ['uniform']")
flags.DEFINE_float("sigma", 0.5, "Sigma used for truncated normal (t-normal) distribution [0.5]")
...
flags.DEFINE_bool("blur_fade", True, "If true, the gaussian blur on images slowly fades during training [False]")
flags.DEFINE_bool("ipython", False, "Switches to interactive command line")
flags.DEFINE_integer("y_dim", 128, "number of labels [y_dim]")
FLAGS = flags.FLAGS
...
def main(_):
    ...
    for name in flags.FLAGS.__flags:
        print "%-20s:" % name,flags.FLAGS.__flags[name].value
