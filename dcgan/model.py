...
class DCGAN(object):
    def __init__(self, sess, input_height=None, input_width=None, is_crop=False,
                 ...
                 input_fname_pattern='*data*.pkl', checkpoint_dir=None, sample_dir=None,
    ...
    def generator(self, z, y=None, sampler=False):
        with tf.variable_scope('generator') as scope:
            ...
            s_h2, s_w2 = conv_out_size_same(s_h, 2), conv_out_size_same(s_w, 2)         #512
            s_h4, s_w4 = conv_out_size_same(s_h2, 2), conv_out_size_same(s_w2, 2)       #256
            s_h8, s_w8 = conv_out_size_same(s_h4, 2), conv_out_size_same(s_w4, 2)       #128
            s_h16, s_w16 = conv_out_size_same(s_h8, 2), conv_out_size_same(s_w8, 2)     #64
    ...
    def train(self, config):
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                batch_images = batch_images.transpose((0, 2, 3, 1)) #这里进行了维度转换！——————————————————————————
                if self.has_labels:
                ...
                counter += 1
                print("Epoch: [%2d/%2d] [%4d/%4d] [%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f, sigma: %4f"
                      % (epoch, config.epoch, idx, batch_idxs, counter, time.time() - start_time, errD_fake+errD_real, errG, sigma_used))
