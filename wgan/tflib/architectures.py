...
def Generator_Resnet_32(cfg, n_samples, labels, noise=None, is_training=True):
    if noise is None:
        noise = tf.random_normal([n_samples, 768]) # 其实G的真正输入在这，就是默认输入就是G的输入最正宗的来源；noise 为2D向量，shape1比较重要
    ...
    if cfg.LAYER_COND:
        ...
        add_dim = cfg.N_LABELS # number of cluster; y 的深度和N_LABELS是一致的
                                   # (name,inputdim,outputdim,inputs,...):('Generator.Input',inputdim=768,outputdim=2048,input= noise)  tf.cast(noise_shape[1],tf.int32)
    output = lib.ops.linear.Linear('Generator.Input',  768 + add_dim, 4 * 4 * cfg.DIM_G, noise) # matmul noise with uniform/Gaussion distribution; noise.shape[1] are feature dimension.
...
def Generator_DCGAN_32(cfg, n_samples, labels, noise=None, is_training=True):
    if noise is None:
...
def Generator_Resnet_128(cfg, n_samples, labels, noise=None, is_training=True):
    if noise is None:
        noise = tf.random_normal([n_samples, 128]) # normalize distribution tensor 产生128维的噪声
    add_dim = 0
    if cfg.LAYER_COND:
        y = labels
        noise = tflib.ops.concat.concat([noise, y], 1) # 把128维噪声和label拼接起来
        add_dim = cfg.N_LABELS
    output = lib.ops.linear.Linear('Generator.Input', 128 + add_dim, 4*4*DIM_G_4, noise) # noise(n_samples,512) * uniform weight matrix (512,512) = (n_samples,512)
...
def Discriminator_Resnet_128(cfg, inputs, labels, is_training=True):
    ...
    if cfg.LAYER_COND:
        output = tflib.ops.concat.concat([output, y], 1)       # 512    +   128 = 640 
    ...
    output_wgan = tf.reshape(output_wgan, [-1]) # [-1]也是展平的意思
    if cfg.CONDITIONAL and cfg.ACGAN:                                     
        output_acgan = lib.ops.linear.Linear('Discriminator.ACGANOutput', DIM_D_8, cfg.N_LABELS, output)
        return output_wgan, output_acgan
    else:
        return output_wgan, None  # D的输出应该为[batch_size,-1],即每个样本被认为为真的概率